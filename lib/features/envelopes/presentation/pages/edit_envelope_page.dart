...
import 'package:kakeibo_pro/features/envelopes/presentation/providers/edit_envelope_notifier.dart';
import 'package:kakeibo_pro/features/envelopes/presentation/widgets/emoji_picker_sheet.dart';

class EditEnvelopePage extends ConsumerStatefulWidget {
  final Envelope envelope;

  const EditEnvelopePage({
    super.key,
    required this.envelope,
  });

  @override
  ConsumerState<EditEnvelopePage> createState() => _EditEnvelopePageState();
}

class _EditEnvelopePageState extends ConsumerState<EditEnvelopePage> {
  ...
  void initState() {
    ...
    _budgetController.text = widget.envelope.monthlyBudget.toInt().toString();
  }
  ...
  Widget build(BuildContext context) {
    final state = ref.watch(editEnvelopeProvider(widget.envelope));
    final notifier =
        ref.read(editEnvelopeProvider(widget.envelope).notifier);

    // Regresar automáticamente al guardar con éxito
    ref.listen(editEnvelopeProvider(widget.envelope), (prev, next) {
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            // ── Gasto actual (informativo, no editable) ───────────────────
            const SizedBox(height: 28),
            Container(
                ...
                children: [
                  ...
                  Expanded(
                    child: Text(
                      ...
                      '${CurrencyFormatter.format(widget.envelope.currentSpent)}',
                      style: const TextStyle(
                        color: AppColors.textMuted,
                        fontSize: 13,
                      ),
  ...
  void _showEmojiPicker(
    ...
    EditEnvelopeState state,
    EditEnvelopeNotifier notifier,
  ) {
